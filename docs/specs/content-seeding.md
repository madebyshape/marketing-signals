...
**Command surface.** One argument, the path to a Seed file. One flag, `--dry-run`. Exit code zero on success, non-zero on any failure. Output is one line per Block (created or skipped, with its type and match key), one line per image (uploaded or reused, with its filename), one line per entry an Entries field named (resolved, with its field, slug and title), and one line per field set on the entry itself (set, with its handle and the entry's slug).
