...
**Command surface.** One argument, the path to a Seed file. One flag, `--dry-run`. Exit code zero on success, non-zero on any failure. Output is one line per Block (created or skipped, with its type and match key), one line per image (uploaded or reused, with its filename), one line per entry an Entries field named (resolved, with its field, slug and title), one line per category a Categories field named (resolved or created, with its field, title and group), and one line per field set on the entry itself (set, with its handle and the entry's slug).

**Seed shape.** A JSON object with `entry` (a slug; `home` is accepted for the Home entry), `field` (a Matrix field handle on that entry, default `blocks`), an optional `volume` (asset volume handle, default `images`), the optional entry keys `section`, `type`, `title`, `parent` and `postDate` described under Creating entries, an optional `replace` described under Replacing Blocks, an optional `fields`, a map of field handle to value set on the entry itself and described under Entry fields, and `blocks`, a list. A Seed carries `blocks`, `fields`, or both. Each Block has `type` (an entry type handle), `fields`, a map of field handle to value, and an optional `after`, the entry type handle of the Block it should follow. The same Block structure is used recursively for nested Matrix values; `after` applies to top-level Blocks only. The shape, as agreed in the grilling session:
...
- Categories: a list of Category titles, in the order the categories should appear. Each is found by title in the field's own group, and a title the group does not hold is created there, so a Seed carries everything it needs. A field whose source is not a group is an error naming the field and its source. A dry run resolves, reports and creates nothing. The Case Study Grid spec is its first caller.
...
**Entry fields.** A top-level `fields` map sets fields on the entry the Seed targets or creates, resolved against that entry's own field layout by the same rules as a Block's fields, so an image is matched by filename in the volume and reused or uploaded exactly as it is inside a Block. It was added for the Client Marquee spec, whose Clients are a title and a Logo with no Matrix field to write to at all, which is why `blocks` is optional when `fields` is present. The fields are written before the Blocks and the entry is saved with them; a field is set every run, so a rerun resolves the same asset and writes nothing new. Dry run resolves, reports and writes nothing.

**Creating entries.** Five optional keys let a Seed reach an entry that does not exist yet, added for the Hero Simple spec, whose test content needs case studies that nobody has created. `section` is a section handle: when no entry with the Seed's slug exists in that section, the command creates one, enabled, with `title` as its title and `type` as its entry type, placed under `parent` (a slug in the same section) when given and at the end of the structure otherwise. The output says the entry was created. A rerun finds the entry by slug and section and skips creation. `type` on an entry that already exists switches it to that entry type, reported as its own output line and skipped when the type already matches. `title` on an entry that already exists mends its title, reported and skipped the same way, and needs `section` too so that it names one entry; it was added for the Service List spec, two of whose Services carry a literal line break copied from the design. `postDate` is an ISO 8601 date given to an entry the command creates, reported on its own output line, and ignored when the entry already exists, since these keys never update what is there; it was added for the Blog Carousel spec, whose seeded Blogs show the dates the design shows. Dry run reports what it would create, switch or retitle and writes nothing. Without `section` the entry must exist, as before.

**Replacing Blocks.** An optional `replace` key, added for the Hero Home spec, whose Home page already holds a Hero in a field that takes one entry. When true, the command removes every Block in the target field before adding the Seed's Blocks, reports what it removed, and adds the Blocks as if the field were empty. A rerun removes and adds again and reports the same, so the result is the same each time even though the skip path never applies. Dry run reports what it would remove and add and writes nothing. Without `replace` the field is never emptied, as before.
...
- Updating an entry's slug or parent once it exists. The entry keys create, switch type and mend a title, nothing else.
